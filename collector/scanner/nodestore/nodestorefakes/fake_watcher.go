...
func (fake *FakeWatcher) OnAdd(arg1 *node.Node) {
...
func (fake *FakeWatcher) OnAddCallCount() int {
	fake.onAddMutex.RLock()
	defer fake.onAddMutex.RUnlock()
	return len(fake.onAddArgsForCall)
}

func (fake *FakeWatcher) OnAddCalls(stub func(*node.Node)) {
	fake.onAddMutex.Lock()
	defer fake.onAddMutex.Unlock()
	fake.OnAddStub = stub
}

func (fake *FakeWatcher) OnAddArgsForCall(i int) *node.Node {
	fake.onAddMutex.RLock()
	defer fake.onAddMutex.RUnlock()
...
var _ nodestore.Watcher = new(FakeWatcher)
